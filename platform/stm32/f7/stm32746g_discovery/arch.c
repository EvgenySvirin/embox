...
void arch_idle(void) {
	...
	if (timer_strat_get_next_event(&next_event_ticks) != 0) {
		/* TODO Sleep for a maximum time instead of exit */
		return;
	}

	next_event_cycles = clock_source_ticks2cycles(cs, next_event_ticks);
	if (next_event_cycles > cs->counter_device->mask) {
		next_event_ticks = clock_source_cycles2ticks(cs, cs->counter_device->mask);
	}

	clock_source_set_oneshot(cs);
	clock_source_set_next_event(cs, next_event_ticks);
